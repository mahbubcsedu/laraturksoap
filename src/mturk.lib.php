<?php namespace mahbubcsedu\turksoap-laravel;
...
class MTurkInterface {
	...
	var $validSP = array (
			...
			"SubmitTime",
			"AssignmentStatus" 
	); /* Sort Property for GetAssignmentsForHIT */
	var $validGRHSP = array (
			"Title",
			"Reward",
			"Expiration",
			"CreationTime" 
	); /* Sort Property for GetReviewableHITs */
	var $validGQRSP = array (
			"QualificationTypeId",
			"SubmitTime" 
	); /* Sort Property for GetQualificationRequests */
	var $validSD = array (
			"Ascending",
			"Descending" 
	); /* Sort Direction */
	...
	var $validSMO = array (
			"Reviewable",
			"Reviewing" 
	); /* StatusMatchOption, or now just Status */
	var $validAS = array (
			"Submitted",
			"Approved",
			"Rejected" 
	...
	var $validGQS = array (
			"Granted",
			"Revoked" 
	); /* Status for GetQualificationsForQualificationType */
   ...
   /* Used by Various Calls */
	...
	/* Constructor */
	function __construct($AccessKey, $SecretKey, $sandbox = false, $Version = "2006-08-23") {
	...
	function SetOperation($operation) {
		if (! in_array ( $operation, $this->validOps ))
			return $this->mtError ( "Invalid Operation Type" );
		else
	...
	function SetVar($var, $val) {
		if (isset ( $this->$var ))
			...
			return $this->mtError ( "Invalid Variable" );
		return TRUE;
	}
	...
	function Invoke() {
		if (! $this->Operation)
			return $this->mtError ( "Missing Operation Parameter" );
		elseif (! in_array ( $this->Operation, $this->validOps ))
			return $this->mtError ( "Invalid Operation Type" );
		elseif (! $this->SecretKey || strlen ( $this->SecretKey ) < 32)
			return $this->mtError ( "Missing/Invalid Secret Key" );
		elseif (! $this->AccessKey || strlen ( $this->AccessKey ) < 16)
			return $this->mtError ( "Missing/Invalid Access Key" );
		if (! $this->Timestamp)
			$this->Timestamp = $this->Unix2ISO8601 ( time () );
		...
		if ($this->ResponseGroup)
			$this->QueryData ['ResponseGroup'] = $this->ResponseGroup;
		if ($this->Validate == TRUE)
			$this->QueryData ['Validate'] = "true";
		
		if (! method_exists ( $this, $this->Operation ))
			return $this->mtError ( "Operation Not Yet Supported :(" );
		...
		if (is_string ( $this->RawData )) {
			...
			} elseif (isset ( $cutName ))
				$this->FinalData = $this->ArrayData [$resultName] [$responseName] [$cutName];
			else
				$this->FinalData = $this->ArrayData [$resultName] [$responseName];
			...
			switch ($this->Operation) {
				...
				case "GetReviewableHITs" :
					...
					if ($this->ResultCounter == 1)
						$hits [] = $this->FinalData ['HIT'] ['HITId'];
					elseif ($this->ResultCounter > 1)
						foreach ( $this->FinalData ['HIT'] as $hitkey => $hitgroup )
							$hits [] = $hitgroup ['HITId'] [$hitkey];
					$this->HITList = $hits;
					break;
	...
	function ApproveAssignment() {
		if (! $this->AssignmentId)
			return $this->mtError ( "Missing AssignmentId" );
		else
			$this->QueryData ['AssignmentId'] = $this->AssignmentId;
		if ($this->RequesterFeedback && strlen ( $this->RequesterFeedback ) > 1024)
			return $this->mtError ( "RequesterFeedback entry is too long!" );
		elseif ($this->RequesterFeedback)
			$this->QueryData ['RequesterFeedback'] = $this->RequesterFeedback;
		return $this->mtMakeRequest ();
	}
	
	/* Create a new HIT - SOAP Operation understands HITTypeId */
	function CreateHIT() {
		if ($this->HITTypeId && $this->mtBreaksHTI ())
			return $this->mtError ( "Incompatible mixing of HITTypeId and other values" );
		elseif (! $this->HITTypeId) {
			/* Values only applicable without HITTypeID */
			if (! $this->Title)
				return $this->mtError ( "Missing Title Parameter" );
			elseif (! $this->Description)
				return $this->mtError ( "Missing Description Parameter" );
			elseif (! $this->Amount)
				return $this->mtError ( "Missing Amount Parameter" );
			elseif (! $this->AssignmentDurationInSeconds)
				return $this->mtError ( "Missing AssignmentDurationInSeconds Parameter" );
			...
			if (is_array ( $this->QualificationRequirement )) {
				foreach ( $this->QualificationRequirement as $itr => $val ) {
					$fg = "(Group {$itr})";
					if (! $val ['QualificationTypeId'])
						return $this->mtError ( "Missing QualificationTypeId Parameter {$fg}" );
					elseif (! $val ['Comparator'])
						return $this->mtError ( "Missing Comparator Parameter {$fg}" );
					elseif (! in_array ( $val ['Comparator'], $this->validCPT ))
						return $this->mtError ( "Invalid Comparator Parameter {$fg}" );
					if ($val ['QualificationTypeId'] == "00000000000000000071") {
						/* Locale Value */
						if (! $val ['LocaleValue'])
							return $this->mtError ( "Need a LocaleValue to accompany a Locale Qualification {$fg}" );
					} else {
						/* Regular Value */
						if (! isset ( $val ['IntegerValue'] ) || ! is_numeric ( $val ['IntegerValue'] ))
							return $this->mtError ( "Need an InterValue to accompany a qualification {$fg}" );
					}
					if (isset ( $val ['RequiredToPreview'] ) && ! in_array ( $val ['RequiredToPreview'], $this->validMBR ))
						return $this->mtError ( "Invalid RequiredToPreview value (true/false) {$fg}" );
				}
			}
		}
		
		/* Required values for either type */
		if (! $this->LifetimeInSeconds)
			return $this->mtError ( "Missing LifetimeInSeconds Parameter" );
		elseif (! $this->Question)
			return $this->mtError ( "Missing Question Parameter" );
	...
	function CreateQualificationType() {
		if (! $this->Name)
			return $this->mtError ( "Missing Name Parameter" );
		elseif (! $this->Description)
			return $this->mtError ( "Missing Description Parameter" );
		elseif (! $this->QualificationTypeStatus)
			return $this->mtError ( "Missing QualificationTypeStatus Parameter" );
		elseif (! in_array ( $this->QualificationTypeStatus, $this->validQTS ))
			return $this->mtError ( "Invalid QualificationTypeStatus Value" );
		elseif ($this->AnswerKey && ! $this->Test)
			return $this->mtError ( "AnswerKey cannot be provided without Test!" );
		return $this->mtFakeSoap ();
	}
	...
	function ExtendHIT() {
		if (! $this->HITId)
			...
			$this->QueryData ['HITId'] = $this->HITId;
		if ($this->MaxAssignmentsIncrement)
			$this->QueryData ['MaxAssignmentsIncrement'] = $this->MaxAssignmentsIncrement;
		if ($this->ExpirationIncrementInSeconds)
			$this->QueryData ['ExpirationIncrementInSeconds'] = $this->ExpirationIncrementInSeconds;
		return $this->mtMakeRequest ();
	}
	...
	//this is added later and need to varify operations
	function GetAssignemtnInfo(){
		if(!$this->$AssignmentId)
			return $this->mtError ( "Missing HITId Parameter" );
			else
				$this->QueryData ['AssignmentId'] = $this->$AssignmentId;
	...
	function GetAssignmentsForHIT() {
		if (! $this->HITId)
			...
			$this->QueryData ['HITId'] = $this->HITId;
		if ($this->AssignmentStatus && ! in_array ( $this->AssignmentStatus, $this->$validAS ))
			return $this->mtError ( "Invalid Assignment Status Value (Submitted/Approved/Rejected)" );
		elseif ($this->AssignmentStatus)
			$this->QueryData ['AssignmentStatus'] = $this->AssignmentStatus;
		if ($this->SortProperty && ! in_array ( $this->SortProperty, $this->$validSP ))
			return $this->mtError ( "Invalid Sort Property Value (AcceptTime/SubmitTime/AssignmentStatus" );
		elseif ($this->SortProperty)
			$this->QueryData ['SortProperty'] = $this->SortProperty;
		if ($this->SortDirection && ! in_array ( $this->SortDirection, $this->validSD ))
			return $this->mtError ( "Invalid SortDirection Value (Ascending/Descending)" );
		elseif ($this->SortDirection)
			$this->QueryData ['SortDirection'] = $this->SortDirection;
		if (is_numeric ( $this->PageSize ) && $this->PageSize > 0)
			$this->QueryData ['PageSize'] = $this->PageSize;
		if (is_numeric ( $this->PageNumber ) && $this->PageNumber > 0)
			$this->QueryData ['PageNumber'] = $this->PageNumber;
		return $this->mtMakeRequest ();
	}
	function GetFileUploadURL() {
		if (! $this->AssignmentId)
			...
			$this->QueryData ['AssignmentId'] = $this->AssignmentId;
		if (! $this->QuestionIdentifier)
			return $this->mtError ( "Missing QuestionIdentifier" );
		else
			$this->QueryData ['QuestionIdentifier'] = $this->QuestionIdentifier;
		return $this->mtMakeRequest ();
	}
	...
	function GetQualificationRequests() {
		if (! $this->QualificationTypeId)
			return $this->mtError ( "Missing QualificationTypeId Parameter" );
		else
			$this->QueryData ['QualificationTypeId'] = $this->QualificationTypeId;
		if (is_numeric ( $this->PageSize ) && $this->PageSize > 0)
			$this->QueryData ['PageSize'] = $this->PageSize;
		if (is_numeric ( $this->PageNumber ) && $this->PageNumber > 0)
			$this->QueryData ['PageNumber'] = $this->PageNumber;
		if ($this->SortProperty && ! in_array ( $this->SortProperty, $this->$validGQRSP ))
			return $this->mtError ( "Invalid Sort Property Value (AcceptTime/SubmitTime/AssignmentStatus" );
		elseif ($this->SortProperty)
			$this->QueryData ['SortProperty'] = $this->SortProperty;
		if ($this->SortDirection && ! in_array ( $this->SortDirection, $this->validSD ))
	...
	function GetQualificationScore() {
		if (! $this->QualificationTypeId)
			...
			$this->QueryData ['QualificationTypeId'] = $this->QualificationTypeId;
		if (! $this->SubjectId)
			return $this->mtError ( "Missing SubjectId Parameter" );
		else
	...
	function GetQualificationsForQualificationType() {
		if (! $this->QualificationTypeId)
			...
			$this->QueryData ['QualificationTypeId'] = $this->QualificationTypeId;
		if ($this->Status && ! in_array ( $this->Status, $this->validGQS ))
			return $this->mtError ( "Invalid Status Parameter" );
		else
	...
	function GetRequesterStatistic() {
		if (! $this->Statistic)
			return $this->mtError ( "Missing Statistic Parameter" );
		elseif (! in_array ( $this->Statistic, $this->validStats ))
			return $this->mtError ( "Invalid Statistic Type" );
		else
			$this->QueryData ['Statistic'] = $this->Statistic;
		if ($this->TimePeriod && ! in_array ( $this->TimePeriod, $this->validTP ))
			return $this->mtError ( "Invalid TimePeriod Type" );
		elseif ($this->TimePeriod)
			$this->QueryData ['TimePeriod'] = $this->TimePeriod;
		if (is_numeric ( $this->Count ) && $this->Count > 0)
			$this->QueryData ['Count'] = $this->Count;
		return $this->mtMakeRequest ();
	}
	...
	function GetReviewableHITs() {
		if ($this->HITTypeId)
			$this->QueryData ['HITTypeId'] = $this->HITTypeId;
		if ($this->Status && ! in_array ( $this->Status, $this->validSMO ))
			return $this->mtError ( "Invalid Status Value (Reviewing/Reviewable - PREVIOUSLY StatusMatchOption)" );
		elseif ($this->Status)
			$this->QueryData ['Status'] = $this->Status;
		if (is_numeric ( $this->PageSize ) && $this->PageSize > 0)
			$this->QueryData ['PageSize'] = $this->PageSize;
		if (is_numeric ( $this->PageNumber ) && $this->PageNumber > 0)
			$this->QueryData ['PageNumber'] = $this->PageNumber;
		if ($this->SortProperty && ! in_array ( $this->SortProperty, $this->validGRHSP ))
			return $this->mtError ( "Invalid SortProperty Value (Title/Reward/Expiration/CreationTime)" );
		elseif ($this->SortProperty)
			$this->QueryData ['SortProperty'] = $this->SortProperty;
		if ($this->SortDirection && ! in_array ( $this->SortDirection, $this->validSD ))
			return $this->mtError ( "Invalid SortDirection Value (Ascending/Descending)" );
	...
	function GrantBonus() {
		if (! $this->WorkerId)
			return $this->mtError ( "Missing WorkerId Parameter(s)" );
		elseif (! $this->AssignmentId)
			return $this->mtError ( "Missing AssignmentId" );
		elseif (! $this->BonusAmount)
			return $this->mtError ( "Missing BonusAmount" );
	...
	function GrantQualification() {
		if (! $this->QualificationRequestId)
			return $this->mtError ( "Missing QualificationRequestId Parameter" );
		else
			$this->QueryData ['QualificationRequestId'] = $this->QualificationRequestId;
		if (! is_numeric ( $this->IntegerValue ))
			return $this->mtError ( "Missing IntegerValue Parameter" );
		else
			$this->QueryData ['IntegerValue'] = $this->Value;
		return $this->mtMakeRequest ();
	}
	...
	function Help() {
		if (! $this->HelpType)
			return $this->mtError ( "Missing HelpType Parameter" );
		elseif (! in_array ( $this->HelpType, $this->validHT ))
			return $this->mtError ( "Invalid HelpType Type" );
		else
			$this->QueryData ['HelpType'] = $this->HelpType;
		if (! $this->About)
			return $this->mtError ( "Missing About Parameter" );
		elseif (! in_array ( $this->About, $this->validOps ))
			return $this->mtError ( "Invalid About Type" );
		else
			$this->QueryData ['About'] = $this->About;
		return $this->mtMakeRequest ();
	}
	...
	function NotifyWorkers() {
		if (! $this->Subject)
			return $this->mtError ( "Missing Subject Parameter" );
		else
			$this->QueryData ['Subject'] = $this->Subject;
		if (! $this->MessageText)
			return $this->mtError ( "Missing MessageText Parameter" );
		else
			$this->QueryData ['MessageText'] = $this->MessageText;
		if (! $this->WorkerId)
			return $this->mtError ( "Missing WorkerId Parameter(s)" );
		elseif (is_array ( $this->WorkerId ))
			foreach ( $this->WorkerId as $wk => $wi )
				$this->QueryData [("WorkerId." . $wk + 1)] = $wi;
		else
			$this->QueryData ['WorkerId'] = $this->WorkerId;
		return $this->mtMakeRequest ();
	}
		...
		if (is_array ( $this->QualificationRequirement )) {
			foreach ( $this->QualificationRequirement as $itr => $val ) {
				...
				elseif (! in_array ( $val ['Comparator'], $this->validCPT ))
					return $this->mtError ( "Invalid Comparator Parameter {$fg}" );
				elseif (! $val ['Value'])
					return $this->mtError ( "Missing Value Parameter {$fg}" );
				elseif (! is_numeric ( $val ['Value'] ))
					return $this->mtError ( "Invalid Value Parameter {$fg}" );
			}
		}
	...
	function RejectQualificationRequest() {
		if (! $this->QualificationRequestId)
			...
			$this->QueryData ['QualificationRequestId'] = $this->QualificationRequestId;
		if ($this->Reason)
			$this->QueryData ['Reason'] = $this->Reason;
		return $this->mtMakeRequest ();
	}
	...
	function SearchQualificationTypes() {
		if ($this->Query)
			$this->QueryData ['Query'] = $this->Query;
		if (is_numeric ( $this->PageSize ) && $this->PageSize > 0)
			$this->QueryData ['PageSize'] = $this->PageSize;
		if (is_numeric ( $this->PageNumber ) && $this->PageNumber > 0)
			$this->QueryData ['PageNumber'] = $this->PageNumber;
		...
		elseif ($this->SortDirection)
			$this->QueryData ['SortDirection'] = $this->SortDirection;
		if ($this->MustBeRequestable && ! in_array ( $this->MustBeRequestable, $this->validMBR ))
			return $this->mtError ( "Invalid MustBeRequestable Value (true/false)" );
		elseif ($this->MustBeRequestable)
			$this->QueryData ['MustBeRequestable'] = $this->MustBeRequestable;
		return $this->mtMakeRequest ();
	}
	function SendTestEventNotification() {
		if (is_array ( $this->Notification )) {
			foreach ( $this->Notification as $itr => $val ) {
				if (! $this->mtCheckNotificationData ( $itr, $val ))
					return false;
				if (! isset ( $val ['Version'] ) && $this->Version)
					$this->Notification [$itr] ['Version'] = $this->Version; /* Helpful */
			}
		}
		if ($this->TestEventType && ! in_array ( $this->TestEventType, $validET ))
			return $this->mtError ( "Invalid TestEventType Value" );
		return $this->mtFakeSoap ();
	}
	function SetHITAsReviewing() {
		if (! $this->HITId)
			...
			$this->QueryData ['HITId'] = $this->HITId;
		if ($this->Revert)
			$this->QueryData ['Revert'] = 'true';
		else
			$this->QueryData ['Revert'] = 'false';
		return $this->mtMakeRequest ();
	}
	...
	function SetHITTypeNotification() {
		if (! $this->HITTypeId)
			return $this->mtError ( "Missing HITTypeId Parameter" );
		if ($this->Active && ! in_array ( $this->Active, $this->validMBR ))
			return $this->mtError ( "Invalid Active Parameter" );
	...
	function UpdateQualificationScore() {
		...
		if (! $this->SubjectId)
			...
			$this->QueryData ['SubjectId'] = $this->SubjectId;
		if (! is_numeric ( $this->IntegerValue ))
			return $this->mtError ( "Invalid IntegerValue Parameter" );
		elseif ($this->IntegerValue < 0 || $this->IntegerValue > 100)
			return $this->mtError ( "Invalid IntegerValue Parameter" );
		else
			$this->QueryData ['IntegerValue'] = $this->IntegerValue;
		return $this->mtMakeRequest ();
	}
	...
	function UpdateQualificationType() {
		if (! $this->QualificationTypeId)
			return $this->mtError ( "Missing QualificationTypeId Parameter" );
		if ($this->QualificationTypeStatus && ! in_array ( $this->QualificationTypeStatus, $this->$validQTS ))
			return $this->mtError ( "Invalid QualificationTypeStatus Value (Active/Inactive)" );
		return $this->mtFakeSoap ();
	}
	...
	function mtMakeRequest() {
		...
		foreach ( $this->QueryData as $a => $b )
			$callData [] = "{$a}=" . urlencode ( $b );
		$this->QueryData = $this->baseURL . "?" . implode ( $callData, "&" );
		$this->QueryString = $this->QueryData;
		$handler = @fopen ( $this->QueryData, "r" );
		if (! $handler)
			return $this->mtError ( "Failed to contact host!" );
		else {
			$data = "";
			while ( ! feof ( $handler ) )
				$data .= fgets ( $handler, 2048 );
			fclose ( $handler );
			if (strlen ( $data ) < 4) {
	...
	function mtFakeSoap() {
      ...
      /* Qualification List */
		...
		$ch = curl_init ();
		curl_setopt ( $ch, CURLOPT_USERAGENT, 'Mahbubur Rahman SOAP Amazon Mechanical Turk' );
	...
	function mtCheckNotificationData($iteration, $data) {
		if (! isset ( $data ['Destination'] ))
			return $this->mtError ( "Missing Destination Value (Notification Level {$iteration})" );
		elseif (! isset ( $data ['Transport'] ))
			return $this->mtError ( "Missing Transport Value (Notification Level {$iteration} - Email/SOAP/REST)" );
		elseif (! isset ( $data ['EventType'] ))
			return $this->mtError ( "Missing EventType Value (Notification Level {$iteration})" );
		else {
			switch ($data ['Transport']) {
				case "Email" :
					if (! $this->mtValidEmail ( $data ['Destination'] ))
						return $this->mtError ( "Invalid email address as destination (Notification Level {$iteration})" );
					break;
				case "SOAP" :
				case "REST" :
					if (! $this->mtValidHTTP ( $data ['Destination'] ))
						return $this->mtError ( "Invalid url as destination (Notification Level {$iteration})" );
					break;
			}
		}
		...
		if (is_array ( $data ['EventType'] )) {
			foreach ( $data ['EventType'] as $checkMe ) {
				if (! in_array ( $checkMe, $validET ))
					return $this->mtError ( "Invalid EventType Value '{$checkMe}' (Notification Level {$iteration})" );
			}
		} elseif (! in_array ( $data ['EventType'], $validET ))
			return $this->mtError ( "Invalid EventType Value (Notification Level {$iteration})" );
		return true;
	}
	...
	function mtCheckPriceData($iteration, $data) {
		if (! isset ( $data ['Amount'] ) || ! is_numeric ( $data ['Amount'] ))
			return $this->mtError ( "Missing/Invalid Amount Specified (Price Level {$iteration})" );
		elseif (! isset ( $data ['CurrencyCode'] ))
			return $this->mtError ( "Missing Currency Code Specified (Price Level {$iteration})" );
		elseif (! in_array ( $data ['CurrencyCode'], $validCC ))
			return $this->mtError ( "Invalid Currency Code Specified (Price Level {$iteration})" );
		return true;
	}
	...
	function PullQualificationRequestList() {
		if (is_array ( $this->QualificationRequestList ))
			return $this->QualificationRequestList;
		else
			return array ();
	}
	function mtPullSource($inputsource) {
		...
		if (! $fp)
			return $this->mtError ( "Unable to access input source!" );
		while ( ! feof ( $fp ) )
			$input .= fgets ( $fp, 1024 );
		fclose ( $fp );
		return $input;
